...
colnames(normal_ssgsea)[4] <- "Dataset"
colnames(cancer_ssgsea)[4] <- "Dataset"
normal_ssgsea <- normal_ssgsea %>%
  dplyr::mutate(Dataset = "TCGA-BRCA Normal") %>%
  dplyr::select(bcr_patient_barcode, aging_up, aging_down, Quartiles, Dataset)
cancer_ssgsea <- cancer_ssgsea %>%
  dplyr::mutate(Dataset = "TCGA-BRCA Cancer") %>%
  dplyr::select(bcr_patient_barcode, aging_up, aging_down, Quartiles, Dataset)
...
# PLOT
###########################################################################################################
ggviolin(combinedDF, x = "Quartiles", y = "aging_up", fill = 'Quartiles', palette = "jco", add = "boxplot",
         ylab = "ssGSEA Score") +
  labs(x = NULL) +
  facet_wrap(facets = ~Type + Dataset, scales = "free_y", nrow = 2) +
  theme_pubr(25, legend = "none") +
  stat_compare_means(method = "wilcox", label = "p.signif", size = 10,
                     label.x = c(1.5, 1.46, 1.5, 0.9))
ggsave("/Users/Yajas/Documents/Elemento/tcga_aging_final/Results/Figures/Fig.4/breast_cancer_aging_ssgsea.eps")
